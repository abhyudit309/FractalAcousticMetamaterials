...
for i in range (0,no_of_cells):
    if abs(mdb.models['Model-1'].parts['AirInsideCarpet'].cells[i].getSize() - a*a*d1) <= tol or abs(mdb.models['Model-1'].parts['AirInsideCarpet'].cells[i].getSize() - a*a*d3) <= tol:
...
mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
    ...
    multiprocessingMode=DEFAULT, name='Job-N{Ni}-{di}-OneSpecimen'.format(Ni=N,di=int(d3*1000)), nodalOutputPrecision=SINGLE
